...
{
    BossFightState bf;
    int *p = bf.positional_trigger_flags;
    int expected_states[NUM_POS_TRIGGERS];
    memset(p, POS_TRIG_INIT, sizeof(bf.positional_trigger_flags));
    memset(expected_states, POS_TRIG_INIT, sizeof(expected_states));
    // Try set invalid values... (should do nothing)
    pos_trigger_set(&bf, -1);
    pos_trigger_set(&bf, NUM_POS_TRIGGERS);
    ...
    for (int i = 0; i < NUM_POS_TRIGGERS; i++)
    {
        if (i % 3 == 0)
        {
            ...
            pos_trigger_set(&bf, i);
        }
        for (int j = 0; j < NUM_POS_TRIGGERS; j++)
        {
            ASSERT(INT_EQ(pos_trigger_state(&bf, j), expected_states[j]));
        }
    }
    // Clear the trigger values
    pos_trigger_clear(&bf);
    for (int i = 0; i < NUM_POS_TRIGGERS; i++)
    {
    ...
    for (int j = 0; j < NUM_POS_TRIGGERS; j++)
    {
        ASSERT(INT_EQ(pos_trigger_state(&bf, j), POS_TRIG_INACTIVE));
    }

    // Check invalid values
    ASSERT(INT_EQ(pos_trigger_state(&bf, -1), POS_TRIG_INIT));
    ASSERT(INT_EQ(pos_trigger_state(&bf, NUM_POS_TRIGGERS), POS_TRIG_INIT));
}
